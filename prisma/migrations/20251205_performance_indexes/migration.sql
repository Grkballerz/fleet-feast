...
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_vendors_cuisine_price_rating"
ON "vendors" ("cuisine_type", "price_range", "status")
WHERE "deleted_at" IS NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_vendors_capacity_search"
ON "vendors" ("capacity_min", "capacity_max", "status")
WHERE "deleted_at" IS NULL AND "status" = 'APPROVED';

-- Full-text search index for vendor search
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_vendors_fts"
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_vendor_menus_fts"
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_bookings_event_upcoming"
ON "bookings" ("event_date", "status")
WHERE "event_date" >= CURRENT_DATE;

-- Reviews table: Aggregate queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_reviews_reviewee_rating"
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_availability_vendor_date_range"
ON "availability" ("vendor_id", "date", "is_available")
WHERE "is_available" = true;

-- Payments table: Escrow release queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_payments_release_queue"
ON "payments" ("status", "captured_at")
WHERE "status" = 'CAPTURED' AND "released_at" IS NULL;

-- Messages table: Conversation queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_messages_booking_unread"
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_quote_requests_active"
ON "quote_requests" ("status", "expires_at")
WHERE "status" = 'OPEN';

-- Composite index for vendor search with location
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_vendors_search_composite"
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_vendors_listing_covering"
ON "vendors" (
  "id",
  "business_name",
  "cuisine_type",
